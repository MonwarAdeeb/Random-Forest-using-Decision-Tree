...
The Random Forest algorithm implemented here reuses some functions from the [Decision Tree implementation]
Generally, it may take different bootstrap sample sizes `n`, different numbers of random features `d` (random subspace sizes), different numbers of random splits `s`, different numbers of decision trees `k`, and different decision tree maximum depths `maxdepth`.
...
├── breast_cancer.py         # Training and Testing on Breast Cancer Wisconsin (Diagnostic) Dataset
└── car_evaluation.py        # Training and Testing on Car Evaluation Dataset
```
